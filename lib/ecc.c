...
#ifdef WITH_SGX
extern randombytes(unsigned char *buf, unsigned long len);

// Unused in GPM's chaincode, but added just for compiling.
static int getRandomNumber(uint64_t *p_vli)
{
    randombytes(*p_vli, ECC_BYTES);
    return 1;
}
#else
...
/* ------ PDID-related functions ------ */

// Q = x*P multiplication from their byte representation
int pdid_ecc_mult(uint8_t Q[PDID_ECC_POINT_BYTES], const uint8_t n[PDID_ECC_SCALAR_BYTES], const uint8_t P[PDID_ECC_POINT_BYTES])
{
    EccPoint l_P;
    uint64_t l_n[NUM_ECC_DIGITS];
    
    ecc_point_decompress(&l_P, P);
    ecc_bytes2native(l_n, n);
    
    EccPoint l_Q;
    EccPoint_mult(&l_Q, &l_P, l_n, NULL);
    
    ecc_native2bytes(Q + 1, l_Q.x);
    Q[0] = 2 + (l_Q.y[0] & 0x01);
    
    return !EccPoint_isZero(&l_Q);
}

// Invertion from wire representation
int pdid_ecc_inv(uint8_t inv_n[PDID_ECC_SCALAR_BYTES], const uint8_t n[PDID_ECC_SCALAR_BYTES])
{
    uint64_t l_n[NUM_ECC_DIGITS];
    uint64_t l_inv_n[NUM_ECC_DIGITS];

	ecc_bytes2native(l_n, n);
	vli_modInv(l_inv_n, l_n, curve_n);
    ecc_native2bytes(inv_n, l_inv_n);
}

// HMVQ, returning point pSK (needed to be hashed afterwards)
int pdid_ecc_hmqv(uint8_t *X, uint8_t *x, uint8_t *P, uint8_t *p, uint8_t *e1, uint8_t *e2, uint8_t *pSK){
    // convert inputs
    EccPoint l_X, l_P;
    uint64_t l_x[NUM_ECC_DIGITS], l_p[NUM_ECC_DIGITS];
    uint64_t l_e1[NUM_ECC_DIGITS],l_e2[NUM_ECC_DIGITS];
    ecc_point_decompress(&l_X, X);
    ecc_point_decompress(&l_P, P);
    ecc_bytes2native(l_x, x);
    ecc_bytes2native(l_p, p);
    ecc_bytes2native(l_e1, e1);
    ecc_bytes2native(l_e2, e2);
    
    // Pe1 <- P^e1
    EccPoint l_Pe1;
    EccPoint_mult(&l_Pe1, &l_P, l_e1, NULL);

    // X <- X*Pe1
    uint64_t z[NUM_ECC_DIGITS];
    vli_modSub(z, l_X.x, l_Pe1.x, curve_p); /* Z = x2 - x1 */
    XYcZ_add(l_Pe1.x, l_Pe1.y, l_X.x, l_X.y);
    vli_modInv(z, z, curve_p); /* Z = 1/Z */
    apply_z(l_X.x, l_X.y, z);

    // exp <- x + e2*p
    uint64_t l_e2p[NUM_ECC_DIGITS];
    vli_modMult(l_e2p, l_e2, l_p, curve_n);
    uint64_t l_exp[NUM_ECC_DIGITS];
    vli_modAdd(l_exp, l_x, l_e2p, curve_n);

    // SK <- X^exp
    EccPoint l_SK;
    EccPoint_mult(&l_SK, &l_X, l_exp, NULL);

    // encode
    ecc_native2bytes(pSK + 1, l_SK.x);
    pSK[0] = 2 + (l_SK.y[0] & 0x01);
    
    return !EccPoint_isZero(&l_SK);
}

// Dummy hash function required for OPRF
int pdid_hash_prime(uint8_t output[PDID_ECC_POINT_BYTES], const unsigned char *str)
{
    uint8_t input[ECC_BYTES];
    uint64_t l_private[NUM_ECC_DIGITS];
    EccPoint l_public;
    unsigned l_tries = 0;
    ...
    memset(input, 0, ECC_BYTES);
    memcpy(input, str, len);
    
    do
    {
        ecc_bytes2native(&l_private, input);
        ...
        input[ECC_BYTES-1] += 1; // safe since MAX_TRIES = 16

    } while(EccPoint_isZero(&l_public));
    
    ecc_native2bytes(output + 1, l_public.x);
    output[0] = 2 + (l_public.y[0] & 0x01);
    return 1;
}

int pdid_ecc_random_secret(uint8_t *x){
	uint8_t _P[PDID_ECC_POINT_BYTES];
	return ecc_make_key(_P, x);
}
